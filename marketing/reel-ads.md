# Cleano Reel Ads

## Reel 1: Main Sales Reel

Length: 25 seconds

Format: vertical 9:16

Goal: convince people to buy the pack today.

Audience: house owners, shop owners, families, and people who want to help Tunisia look cleaner.

### Core Message

Do not sell only a trash grabber. Sell a simple action:

> Clean in front of your door, make Tunisia cleaner, and feel proud.

### Voiceover

```text
شنوة يصير كان كل واحد فينا ينظّف 10 دقايق قدّام دارو؟

مع Cleano Pack، عندك عصا التقاط، قفازات، وأكياس.

تنظّف بلا ما تلمس الزبلة، بلا تعب، وبطريقة سهلة.

عمّر الكيس، صوّر قبل وبعد، واربح نقاط.

ما عندكش وقت؟ أهديها لبوك ولا جدك.

نظّف قدّام دارك، تونس تولّي أنظف.
اطلب الباك اليوم.
```

### On-Screen Text

Use big captions, 2-5 words each:

```text
تونس تبدأ من قدّام دارك
Pack فيه عصا + قفازات + أكياس
تنظّف بلا ما تلمس الزبلة
كيس معمّر = نقاط
صوّر قبل / بعد
اطلب الباك اليوم
```

### Scene Timeline

| Time | Shot | Notes |
| --- | --- | --- |
| 0-2s | Quick dirty corner before shot | Add text: "تونس تبدأ من قدّام دارك" |
| 2-5s | Show the pack on a table | Grabber, gloves, bags must be clear |
| 5-10s | Person picks litter with grabber | Close shot of tool working |
| 10-14s | Full bag tied and placed aside | Show progress and satisfaction |
| 14-18s | Phone with before/after photos | This explains the points platform |
| 18-21s | Older father/grandfather smiling with kit | Emotional gift angle |
| 21-25s | Clean area + product pack + CTA | End with brand and order line |

### Music Direction

- Upbeat Tunisian-friendly rhythm.
- Positive, clean, not dramatic.
- Keep voiceover louder than music.

### Editing Notes

- Use fast cuts every 2-4 seconds.
- Add subtitles for every voiceover line.
- Show the product in the first 5 seconds.
- Do not show huge dirty scenes; keep it solvable and motivating.
- End with the website or page name.

### Caption For Facebook / Instagram / TikTok

```text
ما تستناش شكون ينظّف.
10 دقايق قدّام دارك تنجم تبدّل الحومة.

مع Cleano Pack: عصا التقاط، قفازات، أكياس، وتحدّي نقاط.

نظّف قدّام دارك، تونس تولّي أنظف.
```

Hashtags:

```text
#CleanoTN #نظف_تونس #TunisiePropre #تونس_نظيفة #CleanTunisia
```

## Reel 2: Retired Dad Gift Angle

Length: 20 seconds

### Hook

```text
بوك متقاعد ويحب يتحرك؟
أهديه حاجة تنفعو وتنفع تونس.
```

### Voiceover

```text
ما عندكش وقت تنظّف قدّام دارك؟

أهدي Cleano Pack لبوك ولا جدك.

حركة خفيفة، نشاط، وفرحة بالمساهمة.

كل كيس معمّر يعاون الحومة ويعطي نقاط.

نظّف قدّام دارك، تونس تولّي أنظف.
```

### Visuals

1. Son gives the pack to father.
2. Father opens the pack.
3. Father uses grabber safely.
4. Before/after shot.
5. Product + order CTA.

## Reel 3: Shop Owner Angle

Length: 20 seconds

### Hook

```text
واجهة الحانوت هي أول impression.
```

### Voiceover

```text
حانوتك يبان أنظف، الناس تثق أكثر.

مع Shop Pack، نظّف قدّام الواجهة كل صباح في 10 دقايق.

عصا التقاط، قفازات، وأكياس.

نظافة الحانوت تبدأ من الباب.
```

### Visuals

1. Storefront before opening.
2. Owner cleans quickly.
3. Clean storefront after.
4. Sticker or product pack.
5. CTA: "اطلب Shop Pack".
